...
# Script to create AWS Lambda deployment package for Payment Query Agent

echo "Creating deployment package for Payment Query Agent..."
...
# Create ZIP file
cd $PACKAGE_DIR
zip -r ../payment-query-agent.zip .
cd ..

# Clean up
rm -rf $PACKAGE_DIR

echo "Deployment package created: payment-query-agent.zip"
echo ""
echo "This ZIP file can be uploaded to AWS Lambda."
echo ""
echo "Next steps:"
echo "1. Upload payment-query-agent.zip to AWS Lambda"
